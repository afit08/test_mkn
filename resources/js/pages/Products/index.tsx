...
export default function Index() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const url = `${apiUrl}?page=${page}&search=${encodeURIComponent(search)}&sort_by=${sortBy}&sort_order=${sortOrder}`;
            ...
            <table className="w-full border-collapse border border-gray-300">
              <thead>
                <tr className="bg-gray-100">
                  ...
                  <th className="border border-gray-300 px-4 py-2">Stok Awal</th>
                  <th className="border border-gray-300 px-4 py-2">Total Masuk</th>
                  <th className="border border-gray-300 px-4 py-2">Total Keluar</th>
                  <th
                    className="border border-gray-300 px-4 py-2 cursor-pointer"
                    onClick={() => handleSort('total')}
                  >
                    Total Stok {sortBy === 'total' && (sortOrder === 'asc' ? '▲' : '▼')}
                  </th>
                ...
                {products.length === 0 ? (
                  ...
                  products.map((product, index) => (
                    <tr key={product.id}>
                      ...
                      <td className="border border-gray-300 px-4 py-2">{product.stok_awal}</td>
                      <td className="border border-gray-300 px-4 py-2">{product.total_masuk}</td>
                      <td className="border border-gray-300 px-4 py-2">{product.total_keluar}</td>
